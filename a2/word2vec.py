...
def negSamplingLossAndGradient(
    ...
    gradOutsideVecs[outsideWordIdx] += centerWordVec * (sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec)) - 1.0)
    gradCenterVec += outsideVectors[outsideWordIdx] * (sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec)) - 1.0)

    for k in range(K):
        neg_sample = indices[k+1]
        loss -= np.log(sigmoid(np.dot(-outsideVectors[neg_sample], centerWordVec)))
        gradOutsideVecs[neg_sample] -=  centerWordVec * (sigmoid(np.dot(-outsideVectors[neg_sample], centerWordVec)) - 1.0)
        gradCenterVec -= outsideVectors[neg_sample] * (sigmoid(np.dot(-outsideVectors[neg_sample], centerWordVec)) - 1.0)
